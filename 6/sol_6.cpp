#include <string>
#include <algorithm>
#include <vector>
#include <set>

#include "../utility.h"

namespace Day6
{
    using TNum = int64_t;
    
    constexpr char ADD{ '+' };
    constexpr char MULTIPLY{ '*' };

    struct Problem {
        std::vector<TNum> values;
        char op;
    };


    std::vector<Problem> get_problems(const std::string& file_path);
    TNum solve_problem(const Problem& p);

    TNum sol_6_1(const std::string &file_path)
    {
        auto problem_list = get_problems(file_path);
        TNum grand_total{ };
        
        for (const auto& p : problem_list)
        {
            grand_total += solve_problem(p);
        }

        return grand_total;
    }


    int sol_6_2(const std::string &file_path)
    {
        

        return 0;
    }

    TNum solve_problem(const Problem& p)
    {
        TNum solution{ };

        if (p.op == ADD)
        {
            std::cout << "ADD:" << std::endl;
            for (auto v : p.values)
            {
                std::cout << v << std::endl;
                solution += v;
            }
        }
        else
        {
            std::cout << "Multiply:" << std::endl;
            solution = 1;
            for (auto v : p.values)
            {
                std::cout << v << std::endl;
                solution *= v;
            }
        }
        std::cout << "result: " << solution << std::endl;

        return solution;
    }

    std::vector<Problem> get_problems(const std::string& file_path)
    {
        std::vector<std::string> input_data{};
        
        std::fstream input_file;
        input_file.open(file_path,std::ios::in);
        if (input_file.is_open())
        {
            std::string input_line;
            while(getline(input_file, input_line))
            {
                input_data.push_back(input_line);
            }
            input_file.close();
        }
        
        // get number of problems and retrieve operators
        // retrieve operators
        auto& operator_str = input_data[input_data.size()-1];
        std::vector<char> operator_list{ };
        for (auto i=0; i<operator_str.length(); ++i)
        {
            if (operator_str[i] == ' ')
            {
                continue;
            }
            operator_list.push_back(operator_str[i]);
        }
        

        std::vector<Problem> problem_list(operator_list.size(), Problem{});
        for (auto i=0; i<input_data.size()-1; ++i) {
            auto numbers = parse_string_to_number_vec<TNum>(input_data[i]);
            for (auto i=0; i<numbers.size(); ++i)
            {
                problem_list[i].values.push_back(numbers[i]);
            }
        }
        
        // set operators
        for (auto i=0; i<operator_list.size(); ++i)
        {
            problem_list[i].op = operator_list[i];
        }

        return problem_list;
    }
}
